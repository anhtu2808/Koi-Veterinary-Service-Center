...
import {
  fetchKoisByCustomerIdAPI,
  fetchKoisByAppointmentIdAPI,
...
const Koi = ({
  ...
  const handleViewDetail = (koiId, treatmentId) => {
    if (isAppointment) {
      if (role === "CUSTOMER") {
        navigate(
          `/profile/koidetail/${appointmentId}?treatmentId=${treatmentId}`
        );
      } else {
        navigate(
          `/admin/koidetail/${appointmentId}?treatmentId=${treatmentId}`
        );
      }
    } else {
          ...
          prevList.map((treatment) =>
            treatment.koiTreatmentId === treatmentId
              ? {
                  ...treatment,
                  prescription_id:
                    prescriptionId === "None" ? null : prescriptionId,
                }
              : treatment
          )
        );
          ...
          <table className="table table-hover">
            ...
            <tbody>
              {koiTreatmentList?.map((treatment) => {
                const isSelected = selectedKois?.includes(
                  treatment?.koi?.koiId
                );
                return (
                  <tr
                    key={treatment?.koi?.koiId}
                      ...
                      <div className="d-flex gap-3">
                        ...
                        {isBooking && (
                          <button
                            ...
                            onClick={() =>
                              handleAddKoiToBooking(treatment.koi.koiId)
                            }
                          >
